  ...
  return sideNavList.length === 0 ? 'Loading...' :  (
    <div className=' bg-[#37718E] tracking-wide' >
        <div className='mx-auto p-5 w-11/12 '>
            <div className=' sticky top-20 z-10 p-10 px-20 text-white bg-[#37718E]' >
                <header className='text-4xl font-semibold' >Help & Support</header>
                <p className='text-lg' >Let's take a step ahead and help you better.</p>
            </div>
            <div className='flex  justify-center min-h-screen bg-white p-10 m-2 shadow-lg  ' >
                <div className=' self-start sticky top-48 min-h-screen bg-slate-200 w-1/4  m-4 py-4 hover:cursor-pointer' >
                    {sideNavList.map(item=>(
                        <div key={item.title} onClick={()=>{
                            setSelectedOption(item?.type)
                            setTileHeader(item?.title)
                        }} >
                            <div className= {` text-[#535665] hover:font-semibold m-2 mx-10 mr-0 py-7 pl-10 ${selectedOption === item.type ? ' bg-white font-semibold' : ''}`}>
                                    <p>{item.title}</p>    
                            </div>
                        </div>
                    ))}
                ...
                <hr/>
                <div className='  w-3/4  m-4 p-2' >
                    <div className='text-2xl font-semibold mt-10 mb-5' >{tileHeader}</div>
