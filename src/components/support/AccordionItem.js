...
const AccordionItem = ({ item, selectedIndex }) => {
  ...
  return (
    <div className="border-b-2 border-gray-200 hover:cursor-pointer p-6 text-[#3D4152] font-light " >
        <div className="flex justify-between" >
            <p className="text-lg hover:text-orange-400 " >{item.title}</p>
            { selectedIndex === item.id ? <HiOutlineChevronUp /> : <HiOutlineChevronDown />  }
        </div>
        <div className= {`text-sm w-4/5 ${selectedIndex !== item.id ? 'hidden' : ' my-5  '}`} >
            {
